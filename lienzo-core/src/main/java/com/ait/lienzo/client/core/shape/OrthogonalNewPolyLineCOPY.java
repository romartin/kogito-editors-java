...
public class OrthogonalNewPolyLineCOPY extends AbstractDirectionalMultiPointShape<OrthogonalNewPolyLineCOPY> {
    ...
    public OrthogonalNewPolyLineCOPY(final Point2D... points) {
        this(Point2DArray.fromArrayOfPoint2D(points));
    }

    public OrthogonalNewPolyLineCOPY(final Point2DArray points) {
        super(ShapeType.ORTHOGONAL_POLYLINE);

        setControlPoints(points);
    }

    public OrthogonalNewPolyLineCOPY(final Point2DArray points, final double corner) {
        this(points);
    ...
    public boolean parse() {
        ...
        Point2DArray cps = getControlPoints();
        Point2DArray points = correctBreakDistance(cps, m_breakDistance);
        ...
        if (points.size() > 1) {
            ...
            final NFastDoubleArray opoint = drawOrthogonalLinePoints(points, headDirection, tailDirection, correction, 0, 0, true);
    ...
    public void update(Point2D point, double x, double y) {
        ...
        if (points.size() == 2) {
            ...
            ps.push(points.get(0).copy());
            ps.push(points.get(1).copy());
            Point2DArray inferred = inferOrthogonalSegments(ps);
            Point2DArray result = new Point2DArray();
            result.push(points.get(0));
            for (int i = 0; i < inferred.size(); i++) {
                result.push(inferred.get(i));
            }
    ...
    public void propagate(int index, Function<Integer, Integer> nextIndexSupplier, double dx, double dy) {
        ...
        double offset = i > 1 && (i < points.size() - 1)? 0 : 10;
    ...
    public Point2DArray inferOrthogonalSegments(Point2DArray points) {
        Point2DArray result = new Point2DArray();
        result.push(points.get(0));
        for (int i = 0; i < points.size() - 1; i++) {
            ...
            if (!vertical && !horizontal) {
                ...
                ps.push(p0);
                ps.push(p1);
                NFastDoubleArray p = drawOrthogonalLinePoints(ps, getHeadDirection(), getTailDirection(), 0, 10, 10, true);
                Point2DArray array = Point2DArray.fromNFastDoubleArray(p);
                array = OrthogonalNewPolyLineCOPY.correctComputedPoints(array);
                if (array.size() > 2) {
    ...
    public Point2DArray correctComputedPoints() {
        Point2DArray result = correctComputedPoints(getPoints());
        setPoints(result);
        return result;
    }
    ...
    public OrthogonalNewPolyLineCOPY setCornerRadius(final double radius) {
    ...
    public OrthogonalNewPolyLineCOPY setBreakDistance(double distance) {
    ...
    @Override
    public OrthogonalNewPolyLineCOPY setPoint2DArray(final Point2DArray points) {
    ...
    @Override
    public Shape<OrthogonalNewPolyLineCOPY> copyTo(Shape<OrthogonalNewPolyLineCOPY> other) {
        super.copyTo(other);
        ((OrthogonalNewPolyLineCOPY) other).m_headOffsetPoint = m_headOffsetPoint.copy();
        ((OrthogonalNewPolyLineCOPY) other).m_tailOffsetPoint = m_tailOffsetPoint.copy();
        ((OrthogonalNewPolyLineCOPY) other).m_computedPoint2DArray = m_computedPoint2DArray.copy();
        ((OrthogonalNewPolyLineCOPY) other).m_breakDistance = m_breakDistance;
        ((OrthogonalNewPolyLineCOPY) other).cornerRadius = cornerRadius;

        return other;
    }

    @Override
    public OrthogonalNewPolyLineCOPY cloneLine() {
        OrthogonalNewPolyLineCOPY orthogonalPolyLine = new OrthogonalNewPolyLineCOPY(this.getControlPoints().copy(), cornerRadius);
        return (OrthogonalNewPolyLineCOPY) copyTo(orthogonalPolyLine);
    }
}